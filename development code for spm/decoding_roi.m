function results=decoding_roi(participant_list, ROI_list, CORorANT,results_dir,ROI_expr)
% Tthis function produces an mvpa ROI decoding classification through tdt for a
% list of participants participant_list.
% ROI_list is mandatory, it should be a list of absolute paths to a ROI
% masks or a folder path in participant folder where nifti ROIs are located, starting all with rROI_
% CORorANT = 'COR' or 'ANT' in order to conduct the MVPA with COR left right labels or ANT left right labels. 
% results_dir - where to store the results? In path root>subjID>TD>results_dir
% ROI_expr is a dir expresssion to be used with dir or list_files and find
% the ROIs inside the folder

close all

if exist('participant_list','var')==0; participant_list = {'MV40'}; end %the subject folders

ROI_list='/retinotopic_rois/ROIs/'; %TO BE REMOVED

if exist('ROI_list','var')==0; error('ROI_list missing'); end 
if exist('CORorANT','var')==0; CORorANT = 'COR'; end 
if exist('results_dir','var')==0; results_dir = 'result_roi_COR'; end 
if exist('ROI_expr','var')==0; ROI_expr = 'mrROI_*.nii'; end 

disp('Running ROI decoding with: ')
dispi('ROI_list: ',ROI_list)
dispi('COR or ANT: ',CORorANT)
dispi('Result directory: ',results_dir)

for i=1:numel(participant_list)
    dispi('-------------------------------------')
    dispi('Now running ',participant_list{i})
    main(participant_list{i}, ROI_list, CORorANT,results_dir,ROI_expr)
end
end



function main(participant, ROI_list, CORorANT,results_dir,ROI_expr)
...
addpath('/Users/adrien_chopin/Desktop/decoding_toolbox_v3.991')
...
spm_pipeline_path='/Users/adrien_chopin/Desktop/spm_pipeline';
...
cfg.results.dir = fullfile(spm_pipeline_path,participant,'TDT',results_dir);
% I add some log file starting here
% starts diary
record_notes(cfg.results.dir,'tdt_log')
dispi('Results directory path is ',cfg.results.dir)
...
beta_loc = fullfile(spm_pipeline_path,participant,'spm_stam_thread/GLM/');
...
%cfg.files.mask = fullfile(spm_pipeline_path,participant,'spm_stam_thread/GLM/mask.nii');
%cfg.files.mask = fullfile(spm_pipeline_path,participant,'spm_stam_thread/02_mprage_nifti/gray_mask.nii');
fullfile(spm_pipeline_path,participant,ROI_list)
if exist(fullfile(spm_pipeline_path,participant,ROI_list),'dir') %if ROI_list is a folder, load ROI from there
    ROI_list=list_files(fullfile(spm_pipeline_path,participant,ROI_list),ROI_expr,1)';
end
...
%cfg.searchlight.radius = 9; % this will yield a searchlight radius of 12 units (here: mm). this is the same size as Preston et al, 2008
...
%% we use a custom-made function for measuring decoding success:
%decoding_success_spm(fullfile(cfg.results.dir,['res_',cfg.results.output,'.mat']))
%weirdly enoughm the results from decoding(cfg) do not include roi_names.
%So we load them.
load(fullfile(cfg.results.dir,'res_accuracy_minus_chance.mat'))
disp([{'ROI'},{'Decoding %'},'n voxels';results.roi_names',num2cell(round(100.*results.accuracy_minus_chance.output)./100),num2cell(cellfun(@numel,results.mask_index_each)')])
