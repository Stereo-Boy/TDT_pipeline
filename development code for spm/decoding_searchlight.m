function decoding_searchlight(participant_list, mask, CORorANT,results_dir, sphere_rad)
% Tthis function produces an mvpa decoding classification through tdt for a
% list of participants participant_list.
% If mask is provided as an absolute path to a mask (by default, none, but could be a gray_mask), then the analysis is restricted in that volume.
% If mask is set to 'gray_mask.nii', we will use formula: list_files(fullfile(spm_pipeline_path,participant,'spm_stam_thread/02_mprage_nifti/'),'gray_mask.nii',1)
...
% sphere_rad is the radius in mm of the sphere for searchlight

if exist('participant_list','var')==0; participant_list = {'KM79'}; end %the subject folders
if exist('mask','var')==0; mask = []; end 
if exist('CORorANT','var')==0; CORorANT = 'COR'; end 
if exist('results_dir','var')==0; results_dir = 'result_searchlight_gray_mask_COR'; end 
if exist('sphere_rad','var')==0; sphere_rad = 4.5; end 

disp('Running searchlight decoding with: ')
dispi('Mask: ',mask)
dispi('COR or ANT: ',CORorANT)
dispi('Result directory: ',results_dir)
dispi('Search sphere radius (in mm): ',sphere_rad);

for i=1:numel(participant_list)
    dispi('-------------------------------------')
    dispi('Now running ',participant_list{i})
    main(participant_list{i}, mask, CORorANT,results_dir,sphere_rad)
end
...
function main(participant, mask, CORorANT,results_dir,sphere_rad)
...
cfg.results.dir = check_folder(fullfile(spm_pipeline_path,participant,'TDT',results_dir),0);
...
if strcmp(mask,'gray_mask.nii')
    mask=list_files(fullfile(spm_pipeline_path,participant,'spm_stam_thread/02_mprage_nifti/'),'gray_mask.nii',1);
end
cfg.files.mask = mask;
disp('Using following mask:')
disp(cfg.files.mask)
...
cfg.searchlight.radius = sphere_rad; % this will yield a searchlight diameter of 9 units (here: mm). this is the same size as Preston et al, 2008
